...
export async function executeSearch(
  ...
  if (parsed.intent === 'COMPARISON' && parsed.constraints.comparisonEntities) {
    ...
    // Guard: Identical products comparison
    if (nameA.trim().toLowerCase() === nameB.trim().toLowerCase()) {
      return {
        parsedQuery: parsed,
        status: 'NO_RESULTS',
        items: [],
        message: 'Both products specified for comparison are identical. A comparison requires two distinct products.',
        retrievedAt: new Date().toISOString(),
      };
    }

    // Phase 6.3: Call production Comparison API (/api/comparison-search)
    try {
      const response = await fetch(`/api/comparison-search?q=${encodeURIComponent(query)}`);
      if (response.ok) {
        ...
        } else if (data.status === 'INSUFFICIENT_INPUT') {
          return {
            parsedQuery: parsed,
            status: 'NO_RESULTS',
            items: [],
            message: data.message || 'Could not identify two valid products to compare.',
            retrievedAt: new Date().toISOString(),
          };
      ...
      console.warn('[Discovery Engine] Comparison API call failed, trying local fallback:', apiErr);
    }
