...
const getEnvTag = (countryCode) => {
  ...
  if (typeof import.meta !== 'undefined' && import.meta.env) {
    const viteKey = `VITE_AMAZON_TAG_${code}`;
    const viteIdKey = `VITE_AMAZON_${code}_ID`;
    if (import.meta.env[viteKey]) return import.meta.env[viteKey];
    if (import.meta.env[viteIdKey]) return import.meta.env[viteIdKey];
    if (import.meta.env.VITE_AMAZON_PARTNER_TAG) return import.meta.env.VITE_AMAZON_PARTNER_TAG;
  }
  if (typeof process !== 'undefined' && process.env) {
    const envKey = `AMAZON_TAG_${code}`;
    const envIdKey = `AMAZON_${code}_ID`;
    if (process.env[envKey]) return process.env[envKey];
    if (process.env[envIdKey]) return process.env[envIdKey];
